...
class TopKEvaluator(object):
    ...
    def write_csv(self, score_list, batch_matrix_list, eval_data, idx=0):
        pos_items = eval_data.get_eval_items()
        pos_len_list = eval_data.get_eval_len_list()
        
        score_index = torch.cat(score_list, dim=0)
        topk_index = torch.cat(batch_matrix_list, dim=0)
        full_index = torch.cat([topk_index, score_index], dim = 1)
        ...
        file_path = os.path.join(dir_name, '{}-{}-{}-idx{}-top{}-{}.csv'.format(
            model_name, seed_name, dataset_name, idx, max_k, get_local_time()))
        x_df = pd.DataFrame(full_index)
